...
import com.inswave.appplatform.transaver.elasticsearch.domain.ClientUserTermMonitorLogOriginal;
...
public class ClientUserTermMonitor implements MessageRuleProcessor {

    private List<ClientUserTermMonitorLog> clientUserTermMonitorLogs = new ArrayList<>();

    @Override
    public void parse(String message) throws Exception {

        JavaMelodyMonitor.printInfo("RealtimeEventRuleProcessor Listener Counter", "");
        JavaMelodyMonitor.printInfo("ClientUserTermMonitorRuleProcessor", "");

        message = message.replaceAll("\uFEFF", "");
        Parse parse = new Parse();
        JSONArray jSONArray = parse.getJSONArray(new StringBuffer(message));
        for (Object object : jSONArray) {
            com.inswave.appplatform.log.translate.ClientUserTermMonitor clientUserTermMonitor = new com.inswave.appplatform.log.translate.ClientUserTermMonitor();
            ...
            List<ClientUserTermMonitorLog> tempClientUserTermMonitorLogs = clientUserTermMonitor.reconstructDocument(clientUserTermMonitorLogOriginal);
            for (ClientUserTermMonitorLog clientUserTermMonitorLog : tempClientUserTermMonitorLogs) {
                clientUserTermMonitorLogs.add(clientUserTermMonitorLog);
                if (Config.getInstance().getRunMode1().equals(Constants.RUN_MODE1_TEST) && Config.getInstance().getRunMode3().equals(Constants.RUN_MODE3_INSWAVE) ||
                    ...
                    JavaMelodyMonitor.printInfo("ClientUserTermMonitorRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientUserTermMonitorLog.getTimeCurrent()));
                    JavaMelodyMonitor.printInfo("ClientUserTermMonitorRuleProcessor", "source-->>" + clientUserTermMonitorLog.getSource());
                    JavaMelodyMonitor.printInfo("ClientUserTermMonitorRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientUserTermMonitorLog.getTimeCurrent()) + " deviceId-->>" + clientUserTermMonitorLog.getDeviceId());
                }
            }
        }
    ...
    public List<RuleAlertLog> process(List<Rule> rules) throws NoSuchFieldException, IllegalAccessException, NoSuchMethodException, InvocationTargetException, ScriptException {
        ...
        for (ClientUserTermMonitorLog clientUserTermMonitorLog : clientUserTermMonitorLogs) {
            for (Rule rule : rules) {
                ...
                Field[] fields = ObjectUtil.getFields(clientUserTermMonitorLog);
                for (Field field : fields) {
                    if (patternData.indexOf(field.getName()) != -1) {
                        String fieldValue = ObjectUtil.getFieldValueMethod(clientUserTermMonitorLog, field.getName());
                ...
                if (result) {
                    ...
                    ruleLogs.add(ruleAlertProcessor.getRuleLog(rule, patternData, clientUserTermMonitorLog));
                }
            }
        }
        return ruleLogs;
