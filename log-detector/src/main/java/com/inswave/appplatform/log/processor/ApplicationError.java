...
import com.inswave.appplatform.transaver.elasticsearch.domain.ApplicationErrorLogOriginal;
...
public class ApplicationError implements MessageRuleProcessor {

    private List<ApplicationErrorLog> applicationErrorLogs = new ArrayList<>();

    @Override
    public void parse(String message) throws Exception {

        JavaMelodyMonitor.printInfo("RealtimeEventRuleProcessor Listener Counter", "");
        JavaMelodyMonitor.printInfo("ApplicationErrorRuleProcessor", "");

        message = message.replaceAll("\uFEFF", "");
        Parse parse = new Parse();
        JSONArray jSONArray = parse.getJSONArray(new StringBuffer(message.replaceAll("\uFEFF", "")));
        for (Object object: jSONArray) {
            com.inswave.appplatform.log.translate.ApplicationError applicationError = new com.inswave.appplatform.log.translate.ApplicationError();
            ApplicationErrorLogOriginal applicationErrorLogOriginal = applicationError.getOriginalObject((LinkedHashMap)object);
            List<ApplicationErrorLog> tempApplicationErrorLogs = applicationError.reconstructDocument(applicationErrorLogOriginal);
            for(ApplicationErrorLog applicationErrorLog : tempApplicationErrorLogs) {
                ...
                if (Config.getInstance().getRunMode1().equals(Constants.RUN_MODE1_TEST) && Config.getInstance().getRunMode3().equals(Constants.RUN_MODE3_INSWAVE) ||
                    ...
                    JavaMelodyMonitor.printInfo("ApplicationErrorRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(applicationErrorLog.getTimeCurrent()));
                    JavaMelodyMonitor.printInfo("ApplicationErrorRuleProcessor", "source-->>" + applicationErrorLog.getSource());
                    JavaMelodyMonitor.printInfo("ApplicationErrorRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(applicationErrorLog.getTimeCurrent()) + " deviceId-->>" + applicationErrorLog.getDeviceId());
                }
            }
        }
    }
    ...
    public List<RuleAlertLog> process(List<Rule> rules) throws NoSuchFieldException, IllegalAccessException, NoSuchMethodException, InvocationTargetException, ScriptException {
        ...
        for (ApplicationErrorLog applicationErrorLog : applicationErrorLogs) {
            for (Rule rule : rules) {
                ...
                Field[] fields = ObjectUtil.getFields(applicationErrorLog);
                for (Field field : fields) {
                    if (patternData.indexOf(field.getName()) != -1) {
                        String fieldValue = ObjectUtil.getFieldValueMethod(applicationErrorLog, field.getName());
                ...
                if (result) {
                    ...
                    ruleLogs.add(ruleAlertProcessor.getRuleLog(rule, patternData, applicationErrorLog));
                }
            }
        }
        return ruleLogs;
