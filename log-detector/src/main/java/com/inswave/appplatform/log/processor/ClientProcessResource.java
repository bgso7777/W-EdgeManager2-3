...
import com.inswave.appplatform.transaver.elasticsearch.domain.ClientProcessResourceLogOriginal;
...
public class ClientProcessResource implements MessageRuleProcessor {

    private List<ClientProcessResourceLog> clientProcessResourceLogs = new ArrayList<>();

    @Override
    public void parse(String message) throws Exception {

        JavaMelodyMonitor.printInfo("RealtimeEventRuleProcessor Listener Counter", "");
        JavaMelodyMonitor.printInfo("ClientProcessResourceLogRuleProcessor", "");

        message = message.replaceAll("\uFEFF", "");
        Parse parse = new Parse();
        JSONArray jSONArray = parse.getJSONArray(new StringBuffer(message));
        for (Object object: jSONArray) {
            com.inswave.appplatform.log.translate.ClientProcessResource clientProcessResource = new com.inswave.appplatform.log.translate.ClientProcessResource();
            ...
            List<ClientProcessResourceLog> tempClientProcessResourceLogs = clientProcessResource.reconstructDocument(clientProcessResourceLogOriginal);
            for (ClientProcessResourceLog clientProcessResourceLog : tempClientProcessResourceLogs) {
                ...
                if (Config.getInstance().getRunMode1().equals(Constants.RUN_MODE1_TEST) && Config.getInstance().getRunMode3().equals(Constants.RUN_MODE3_INSWAVE) ||
                    ...
                    JavaMelodyMonitor.printInfo("ClientProcessResourceRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientProcessResourceLog.getTimeCurrent()));
                    JavaMelodyMonitor.printInfo("ClientProcessResourceRuleProcessor", "source-->>" + clientProcessResourceLog.getSource());
                    JavaMelodyMonitor.printInfo("ClientProcessResourceRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientProcessResourceLog.getTimeCurrent()) + " deviceId-->>" + clientProcessResourceLog.getDeviceId());
                }
            }
        }
    ...
    public List<RuleAlertLog> process(List<Rule> rules) throws NoSuchFieldException, IllegalAccessException, NoSuchMethodException, InvocationTargetException, ScriptException {
        ...
        for (ClientProcessResourceLog clientProcessResourceLog : clientProcessResourceLogs) {
            for (Rule rule : rules) {
                ...
                Field[] fields = ObjectUtil.getFields(clientProcessResourceLog);
                for (Field field : fields) {
                    if (patternData.indexOf(field.getName()) != -1) {
                        String fieldValue = ObjectUtil.getFieldValueMethod(clientProcessResourceLog, field.getName());
                ...
                if (result) {
                    ...
                    ruleLogs.add(ruleAlertProcessor.getRuleLog(rule, patternData, clientProcessResourceLog));
                }
            }
        }
        return ruleLogs;
