...
public class ClientProcessCreation implements MessageRuleProcessor {

    private List<ClientProcessCreationLog> clientProcessCreationLogs = new ArrayList<>();

    @Override
    public void parse(String message) throws Exception {

        JavaMelodyMonitor.printInfo("RealtimeEventRuleProcessor Listener Counter", "");
        JavaMelodyMonitor.printInfo("ClientProcessCreationProcessor", "");

        message = message.toString().replace("\uFEFF", "");
        Parse parse = new Parse();
        JSONArray jSONArray = parse.getJSONArray(new StringBuffer(message));
        for (Object object: jSONArray) {
            com.inswave.appplatform.log.translate.ClientProcessCreation clientProcessCreation = new com.inswave.appplatform.log.translate.ClientProcessCreation();
            ...
            List<ClientProcessCreationLog> tempClientProcessCreationLogs = clientProcessCreation.reconstructDocument(clientProcessCreationLogOriginal);
            for (ClientProcessCreationLog clientProcessCreationLog : tempClientProcessCreationLogs) {
                ...
                if (Config.getInstance().getRunMode1().equals(Constants.RUN_MODE1_TEST) && Config.getInstance().getRunMode3().equals(Constants.RUN_MODE3_INSWAVE) ||
                    ...
                    JavaMelodyMonitor.printInfo("ClientProcessCreationProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientProcessCreationLog.getTimeCurrent()));
                    JavaMelodyMonitor.printInfo("ClientProcessCreationProcessor", "source-->>" + clientProcessCreationLog.getSource());
                    JavaMelodyMonitor.printInfo("ClientProcessCreationProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientProcessCreationLog.getTimeCurrent()) + " deviceId-->>" + clientProcessCreationLog.getDeviceId());
                }
            }
        }
    }
    ...
    public List<RuleAlertLog> process(List<Rule> rules) throws NoSuchFieldException, IllegalAccessException, NoSuchMethodException, InvocationTargetException, ScriptException {
        ...
        for (ClientProcessCreationLog clientProcessCreationLog : clientProcessCreationLogs) {
            for (Rule rule : rules) {
                ...
                Field[] fields = ObjectUtil.getFields(clientProcessCreationLog);
                for (Field field : fields) {
                    if (patternData.indexOf(field.getName()) != -1) {
                        String fieldValue = ObjectUtil.getFieldValueMethod(clientProcessCreationLog, field.getName());
                ...
                if (result) {
                    ...
                    ruleLogs.add(ruleAlertProcessor.getRuleLog(rule, patternData, clientProcessCreationLog));
                }
            }
        }
        return ruleLogs;
