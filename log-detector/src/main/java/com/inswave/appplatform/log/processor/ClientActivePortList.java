...
public class ClientActivePortList implements MessageRuleProcessor {

    private List<ClientActivePortListResourceLog> clientActivePortListResourceLogs = new ArrayList<>();

    @Override
    public void parse(String message) throws Exception {

        JavaMelodyMonitor.printInfo("RealtimeEventRuleProcessor Listener Counter", "");
        JavaMelodyMonitor.printInfo("ClientActivePortListProcessor", "");
        ...
        for (Object object: jSONArray) {
            ...
            List<ClientActivePortListResourceLog> tempClientActivePortListResourceLogs = clientActivePortListResource.reconstructDocument(clientActivePortListResourceLogOriginal);
            for(ClientActivePortListResourceLog clientActivePortListResourceLog : tempClientActivePortListResourceLogs) {
                ...
                if (Config.getInstance().getRunMode1().equals(Constants.RUN_MODE1_TEST) && Config.getInstance().getRunMode3().equals(Constants.RUN_MODE3_INSWAVE) ||
                    ...
                    JavaMelodyMonitor.printInfo("ClientActivePortListRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientActivePortListResourceLog.getTimeCurrent()));
                    JavaMelodyMonitor.printInfo("ClientActivePortListRuleProcessor", "source-->>" + clientActivePortListResourceLog.getSource());
                    JavaMelodyMonitor.printInfo("ClientActivePortListRuleProcessor", "date-->>" + (new SimpleDateFormat(ConstantsTranSaver.TAG_DATE_PATTERN_OF_ELASTICSEARCH_INDEX)).format(clientActivePortListResourceLog.getTimeCurrent()) + " deviceId-->>" + clientActivePortListResourceLog.getDeviceId());
                }
            }
        }
    ...
    public List<RuleAlertLog> process(List<Rule> rules) throws NoSuchFieldException, IllegalAccessException, NoSuchMethodException, InvocationTargetException, ScriptException {
        ...
        for (ClientActivePortListResourceLog clientActivePortListResourceLog : clientActivePortListResourceLogs) {
            for (Rule rule : rules) {
                ...
                Field[] fields = ObjectUtil.getFields(clientActivePortListResourceLog);
                for (Field field : fields) {
                    if (patternData.indexOf(field.getName()) != -1) {
                        String fieldValue = ObjectUtil.getFieldValueMethod(clientActivePortListResourceLog, field.getName());
                ...
                if (result) {
                    ...
                    ruleLogs.add(ruleAlertProcessor.getRuleLog(rule, patternData, clientActivePortListResourceLog));
                }
            }
        }
        return ruleLogs;
