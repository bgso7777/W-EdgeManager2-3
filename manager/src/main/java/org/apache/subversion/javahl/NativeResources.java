...
//                                   "but is only " + version);
            log.error("Native library version must be at least {}.{}.0,but is only {}", SVN_VER_MAJOR, SVN_VER_MINOR, version);
        }
...
//                " but the run-time version is " + runtimeVersion);
            log.error("Compile-time Native library version is {} but the run-time version is {}", version, runtimeVersion);
        }
    }
