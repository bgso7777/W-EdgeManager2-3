...
import com.inswave.appplatform.transaver.saver.ClientPerformanceResourceLogger;
...
@Service
public final class ClientPerformanceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientPerformanceResourceLogger clientPerformanceResourceLogger;

    private static final String topicName = "ClientPerformanceLog";
    private static final String groupId = "ClientPerformanceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientperformancelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientPerformanceResourceLogger.insert("ClientPerformanceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientPerformanceResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientPerformanceResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientPerformanceResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientPerformanceResourceLogger.cloneLoggerMonitor();
