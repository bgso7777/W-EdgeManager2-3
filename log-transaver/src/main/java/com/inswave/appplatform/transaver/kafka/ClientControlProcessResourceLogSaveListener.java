...
import com.inswave.appplatform.transaver.saver.ClientControlProcessResourceLogger;
...
@Service
public final class ClientControlProcessResourceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientControlProcessResourceLogger clientControlProcessResourceLogger;

    private static final String topicName = "ClientControlProcessResourceLog";
    private static final String groupId = "ClientControlProcessResourceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientcontrolprocessresourcelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientControlProcessResourceLogger.insert("ClientControlProcessResourceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientControlProcessResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientControlProcessResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientControlProcessResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientControlProcessResourceLogger.cloneLoggerMonitor();
