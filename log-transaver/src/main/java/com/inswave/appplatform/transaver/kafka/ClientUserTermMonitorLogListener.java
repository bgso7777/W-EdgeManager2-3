...
import com.inswave.appplatform.transaver.saver.ClientUserTermMonitorLogger;
...
@Service
public final class ClientUserTermMonitorLogListener extends KafkaListenerMonitor {

    @Autowired
    ClientUserTermMonitorLogger clientUserTermMonitorLogger;

    private static final String topicName = "ClientUserTermMonitorLog";
    private static final String groupId = "ClientUserTermMonitorLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientusertermmonitorloglistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientUserTermMonitorLogger.insert("ClientUserTermMonitorLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientUserTermMonitorLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientUserTermMonitorLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientUserTermMonitorLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientUserTermMonitorLogger.cloneLoggerMonitor();
