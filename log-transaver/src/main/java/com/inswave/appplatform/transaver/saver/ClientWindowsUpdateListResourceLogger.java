...
import com.inswave.appplatform.transaver.elasticsearch.dao.ClientWindowsUpdateListResourceLogDailyRepository;
import com.inswave.appplatform.transaver.elasticsearch.dao.ClientWindowsUpdateListResourceLogRepository;
...
@Service
public class ClientWindowsUpdateListResourceLogger extends LoggerMonitor implements ElasticsearchService {
	...
	public static String className = "ClientWindowsUpdateListResourceLogger";
	public static String currentIndexName = "ClientWindowsUpdateListResourceLog";
	public static String dailyIndexName = "ClientWindowsUpdateListResourceLogDaily";
...
//		Object objectNode = parse.getObjectNode(message);
		...
		for (Object object: jSONArray) {
			...
			super.setRowSize(super.getRowSize()+clientWindowsUpdateListResourceLogOriginal.getClientWindowsUpdateListResourceData().size());
			List<ClientWindowsUpdateListResourceLog> clientWindowsUpdateListResourceLogs = clientWindowsUpdateListResource.reconstructDocument(clientWindowsUpdateListResourceLogOriginal);
			List<ClientWindowsUpdateListResourceLogDaily> clientWindowsUpdateListResourceLogDailies = clientWindowsUpdateListResource.reconstructDailyDocument(clientWindowsUpdateListResourceLogOriginal);
			super.endTimeObjectConvert();

			// deviceId별 current 로그 저장
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			super.beginTimeElasticsearchCurrentInsert();
			ClientWindowsUpdateListResourceLogRepository clientWindowsUpdateListResourceLogRepository = (ClientWindowsUpdateListResourceLogRepository) ElasticsearchDaoPackage.getInstance().getElasticsearchDao(currentIndexName);
			DynamicIndexBean.setIndexName(currentIndexName.toLowerCase());
			clientWindowsUpdateListResourceLogRepository.setIndexName(currentIndexName.toLowerCase());
			if (!elasticsearchHighLevelClient.existIndices(currentIndexName.toLowerCase())) {
				do {
					try {
						clientWindowsUpdateListResourceLogs.get(0).setIndexName(currentIndexName.toLowerCase());
						clientWindowsUpdateListResourceLogRepository.setIndexName(currentIndexName.toLowerCase());
						clientWindowsUpdateListResourceLogRepository.createIndex(clientWindowsUpdateListResourceLogs.get(0));
			...
			try {
//				elasticsearchHighLevelClient.deleteDocument(currentIndexName.toLowerCase(), "deviceId", clientWindowsUpdateListResourceLogOriginal.getDeviceId());
				if(!isCurrentIndexDelete) {
					elasticsearchHighLevelClient.deleteCurrentIndex(currentIndexName.toLowerCase(),clientWindowsUpdateListResourceLogOriginal);
			...
			super.setCurrentRowSize(super.getCurrentRowSize()+clientWindowsUpdateListResourceLogs.size());
			...
			for (ClientWindowsUpdateListResourceLog clientWindowsUpdateListResourceLog : clientWindowsUpdateListResourceLogs) {
				clientWindowsUpdateListResourceLog.setIndexName(currentIndexName.toLowerCase());
				document2s.add(clientWindowsUpdateListResourceLog);
			}
			try {
			...
			// 일자별 로그 저장
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if(Config.getInstance().getLog().getIndexDailylogClientwindowsUpdateListResourceLogSavedDay()>0) {
				super.beginTimeElasticsearchDailyInsert();
				String ip = "", deviceId = "", currentDate = "", timeCreated = "", regDate = "";
				ClientWindowsUpdateListResourceLogDailyRepository clientWindowsUpdateListResourceLogDailyRepository = (ClientWindowsUpdateListResourceLogDailyRepository) ElasticsearchDaoPackage.getInstance().getElasticsearchDao(dailyIndexName);
				document2s = new ArrayList<>();
				HashSet<String> indexNames = new HashSet<>();
				for (ClientWindowsUpdateListResourceLogDaily clientWindowsUpdateListResourceLogDaily : clientWindowsUpdateListResourceLogDailies) {
					try {
						indexName = DateTimeConvertor.getIndexName(dailyIndexName.toLowerCase(), clientWindowsUpdateListResourceLogDaily.getTimeCurrent());
						indexNames.add(indexName);
						clientWindowsUpdateListResourceLogDaily.setIndexName(indexName);
						clientWindowsUpdateListResourceLogDaily.setTimeRegistered(DateTimeConvertor.getTimeRegistered());
						ip = clientWindowsUpdateListResourceLogDaily.getIp();
						deviceId = clientWindowsUpdateListResourceLogDaily.getDeviceId();
						currentDate = DateTimeConvertor.getPrintDate(clientWindowsUpdateListResourceLogDaily.getTimeCurrent());
						timeCreated = DateTimeConvertor.getPrintDate(clientWindowsUpdateListResourceLogDaily.getTimeCreated());
						regDate = DateTimeConvertor.getPrintDate(clientWindowsUpdateListResourceLogDaily.getTimeRegistered());
						clientWindowsUpdateListResourceLogDailyRepository.setIndexName(clientWindowsUpdateListResourceLogDaily.getIndexName());
						document2s.add(clientWindowsUpdateListResourceLogDaily);
					} catch (Exception e) {
						e.printStackTrace();
						super.setExceptionCount(super.getExceptionCount() + 1);
					}
				}
				super.setDailyRowSize(super.getDailyRowSize() + clientWindowsUpdateListResourceLogDailies.size());
				try {
					BulkResponse bulkResponse = elasticsearchHighLevelClient.writeDocument2s(document2s);
					if (bulkResponse.hasFailures())
						logger.error(bulkResponse.buildFailureMessage());
	...
	public static void main(String[] argv) {
		...
		try	{
			messageFile = FileUtil.getFileToStringBuffer("C:\\Temp\\clientWindowsUpdateListResourceLog.message"); // ok
...
//				objectMapper.setDateFormat(dateFormat);
				...
				ClientWindowsUpdateListResource clientWindowsUpdateListResource = new ClientWindowsUpdateListResource();
				ClientWindowsUpdateListResourceLogOriginal clientWindowsUpdateListResourceLogOriginal = clientWindowsUpdateListResource.getOriginalObject((LinkedHashMap)objectNode);

				List<ClientWindowsUpdateListResourceLog> clientWindowsUpdateListResourceLogs = clientWindowsUpdateListResource.reconstructDocument(clientWindowsUpdateListResourceLogOriginal);
				List<ClientWindowsUpdateListResourceLogDaily> clientWindowsUpdateListResourceLogDailies = clientWindowsUpdateListResource.reconstructDailyDocument(clientWindowsUpdateListResourceLogOriginal);

				String regDate = DateTimeConvertor.getPrintDate(clientWindowsUpdateListResourceLogDailies.get(0).getTimeCreated());
				String indexName = DateTimeConvertor.getIndexName(dailyIndexName.toLowerCase(),clientWindowsUpdateListResourceLogDailies.get(0).getTimeCurrent());
