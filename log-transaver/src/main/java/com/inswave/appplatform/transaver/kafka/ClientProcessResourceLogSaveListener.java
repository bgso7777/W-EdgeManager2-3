...
import com.inswave.appplatform.transaver.saver.ClientProcessResourceLogger;
...
@Service
public final class ClientProcessResourceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientProcessResourceLogger clientProcessResourceLogger;

    private static final String topicName = "ClientResourceLog";
    private static final String groupId = "ClientResourceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientprocessresourcelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientProcessResourceLogger.insert("ClientResourceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientProcessResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientProcessResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientProcessResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientProcessResourceLogger.cloneLoggerMonitor();
