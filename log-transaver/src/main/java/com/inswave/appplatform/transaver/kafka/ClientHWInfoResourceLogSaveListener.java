...
import com.inswave.appplatform.transaver.saver.ClientHWInfoResourceLogger;
...
@Service
public final class ClientHWInfoResourceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientHWInfoResourceLogger clientHWInfoResourceLogger;

    private static final String topicName = "ClientHWInfoResourceLog";
    private static final String groupId = "ClientHWInfoResourceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clienthwinforesourcelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientHWInfoResourceLogger.insert("ClientHWInfoResourceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientHWInfoResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientHWInfoResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientHWInfoResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientHWInfoResourceLogger.cloneLoggerMonitor();
