...
import com.inswave.appplatform.transaver.saver.DeviceErrorLogger;
...
@Service
public final class DeviceErrorLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    DeviceErrorLogger deviceErrorLogger;

    private static final String topicName = "DeviceErrorLog";
    private static final String groupId = "DeviceErrorLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.deviceerrorlogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            deviceErrorLogger.insert("DeviceErrorLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            deviceErrorLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            deviceErrorLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                deviceErrorLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = deviceErrorLogger.cloneLoggerMonitor();
