...
import com.inswave.appplatform.transaver.saver.ClientDefragAnalysisResourceLogger;
...
@Service
public final class ClientDefragAnalysisResourceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientDefragAnalysisResourceLogger clientDefragAnalysisResourceLogger;

    private static final String topicName = "ClientDefragAnalysisResourceLog";
    private static final String groupId = "ClientDefragAnalysisResourceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientdefraganalysisresourcelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientDefragAnalysisResourceLogger.insert("ClientDefragAnalysisResourceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientDefragAnalysisResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientDefragAnalysisResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientDefragAnalysisResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientDefragAnalysisResourceLogger.cloneLoggerMonitor();
