...
import com.inswave.appplatform.transaver.saver.IntegrityLogger;
...
@Service
public final class IntegrityLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    IntegrityLogger integrityLogger;

    private static final String topicName = "IntegrityLog";
    private static final String groupId = "IntegrityLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.integritylogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            integrityLogger.insert("IntegrityLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            integrityLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            integrityLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                integrityLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = integrityLogger.cloneLoggerMonitor();
