...
import com.inswave.appplatform.log.translate.ClientHWInfoResource;
...
import com.inswave.appplatform.transaver.elasticsearch.dao.ClientHWInfoResourceLogDailyRepository;
import com.inswave.appplatform.transaver.elasticsearch.dao.ClientHWInfoResourceLogRepository;
...
@Service
public class ClientHWInfoResourceLogger extends LoggerMonitor implements ElasticsearchService {
	...
	public static String className = "ClientHWInfoResourceLogger";
	public static String currentIndexName = "ClientHWInfoResourceLog";
	public static String dailyIndexName = "ClientHWInfoResourceLogDaily";
...
//		Object objectNode = parse.getObjectNode(message);
		...
		for (Object object : jSONArray) {
			...
			super.setRowSize(super.getRowSize() + clientHWInfoResourceLogOriginal.getClientHWInfoResourceData().size());
			List<ClientHWInfoResourceLog> clientHWInfoResourceLogs = clientHWInfoResource.reconstructDocument(clientHWInfoResourceLogOriginal);
			List<ClientHWInfoResourceLogDaily> clientHWInfoResourceLogDailies = clientHWInfoResource.reconstructDailyDocument(clientHWInfoResourceLogOriginal);
			super.endTimeObjectConvert();

			// deviceId별 current 로그 저장
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			super.beginTimeElasticsearchCurrentInsert();
			ClientHWInfoResourceLogRepository clientHWInfoResourceLogRepository = (ClientHWInfoResourceLogRepository) ElasticsearchDaoPackage.getInstance().getElasticsearchDao(currentIndexName);
			DynamicIndexBean.setIndexName(currentIndexName.toLowerCase());
			clientHWInfoResourceLogRepository.setIndexName(currentIndexName.toLowerCase());
			if (!elasticsearchHighLevelClient.existIndices(currentIndexName.toLowerCase())) {
				do {
					try {
						clientHWInfoResourceLogs.get(0).setIndexName(currentIndexName.toLowerCase());
						clientHWInfoResourceLogRepository.setIndexName(currentIndexName.toLowerCase());
						clientHWInfoResourceLogRepository.createIndex(clientHWInfoResourceLogs.get(0));
			...
			try {
//				elasticsearchHighLevelClient.deleteDocument(currentIndexName.toLowerCase(), "deviceId", clientHWInfoResourceLogOriginal.getDeviceId());
				if(!isCurrentIndexDelete) {
					elasticsearchHighLevelClient.deleteCurrentIndex(currentIndexName.toLowerCase(),clientHWInfoResourceLogOriginal);
			...
			super.setCurrentRowSize(super.getCurrentRowSize() + clientHWInfoResourceLogs.size());
			List<Document2> document2s = new ArrayList<>();
			for (ClientHWInfoResourceLog clientHWInfoResourceLog : clientHWInfoResourceLogs) {
				clientHWInfoResourceLog.setIndexName(currentIndexName.toLowerCase());
				document2s.add(clientHWInfoResourceLog);
			}
			try {
			...
			// 일자별 로그 저장
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if (Config.getInstance().getLog().getIndexDailylogClientHwInfoResourceLogSavedDay()>0) {
				super.beginTimeElasticsearchDailyInsert();
				String ip = "", deviceId = "", currentDate = "", timeCreated = "", regDate = "";
				ClientHWInfoResourceLogDailyRepository clientHWInfoResourceLogDailyRepository = (ClientHWInfoResourceLogDailyRepository) ElasticsearchDaoPackage.getInstance().getElasticsearchDao(dailyIndexName);
				HashSet<String> indexNames = new HashSet<>();
				document2s = new ArrayList<>();
				for (ClientHWInfoResourceLogDaily clientHWInfoResourceLogDaily : clientHWInfoResourceLogDailies) {
					try {
						indexName = DateTimeConvertor.getIndexName(dailyIndexName.toLowerCase(), clientHWInfoResourceLogDaily.getTimeCurrent());
						indexNames.add(indexName);
						clientHWInfoResourceLogDaily.setIndexName(indexName);
						clientHWInfoResourceLogDaily.setTimeRegistered(DateTimeConvertor.getTimeRegistered());
						ip = clientHWInfoResourceLogDaily.getIp();
						deviceId = clientHWInfoResourceLogDaily.getDeviceId();
						currentDate = DateTimeConvertor.getPrintDate(clientHWInfoResourceLogDaily.getTimeCurrent());
						timeCreated = DateTimeConvertor.getPrintDate(clientHWInfoResourceLogDaily.getTimeCreated());
						regDate = DateTimeConvertor.getPrintDate(clientHWInfoResourceLogDaily.getTimeRegistered());
						document2s.add(clientHWInfoResourceLogDaily);
					} catch (Exception e) {
						e.printStackTrace();
						super.setExceptionCount(super.getExceptionCount() + 1);
					}
				}
				super.setDailyRowSize(super.getDailyRowSize() + clientHWInfoResourceLogDailies.size());
				try {
					BulkResponse bulkResponse = elasticsearchHighLevelClient.writeDocument2s(document2s);
					if (bulkResponse.hasFailures())
						logger.error(bulkResponse.buildFailureMessage());
	...
	public static void main(String[] argv) {
		...
		try	{
			messageFile = FileUtil.getFileToStringBuffer("C:\\Temp\\ClientHWInfoResourceLog.message"); // ok
			...
			for (int i = 0; i < messages.length; i++) {
				...
				ClientHWInfoResource clientHWInfoResource = new ClientHWInfoResource();
				ClientHWInfoResourceLogOriginal clientHWInfoResourceLogOriginal = clientHWInfoResource.getOriginalObject((LinkedHashMap)objectNode);
				List<ClientHWInfoResourceLog> clientHWInfoResourceLogs = clientHWInfoResource.reconstructDocument(clientHWInfoResourceLogOriginal);
				List<ClientHWInfoResourceLogDaily> clientHWInfoResourceLogDailies = clientHWInfoResource.reconstructDailyDocument(clientHWInfoResourceLogOriginal);
				System.out.println();
			}
