...
import com.inswave.appplatform.transaver.saver.HWErrorLogger;
...
@Service
public final class HWErrorLogSavealtimeListener extends KafkaListenerMonitor {

    @Autowired
    HWErrorLogger hWErrorLogger;

    private static final String topicName = "HWErrorLog";
    private static final String groupId = "HWErrorLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.hwerrorlogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            hWErrorLogger.insert("HWErrorLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            hWErrorLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            hWErrorLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                hWErrorLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = hWErrorLogger.cloneLoggerMonitor();
