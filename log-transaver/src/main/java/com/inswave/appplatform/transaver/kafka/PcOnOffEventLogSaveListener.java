...
import com.inswave.appplatform.transaver.saver.PcOnOffEventLogger;
...
@Service
public final class PcOnOffEventLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    PcOnOffEventLogger pcOnOffEventLogger;

    private static final String topicName = "PcOnOffEventLog";
    private static final String groupId = "PcOnOffEventLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.pconoffeventlogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            pcOnOffEventLogger.insert("PcOnOffEventLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            pcOnOffEventLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            pcOnOffEventLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                pcOnOffEventLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = pcOnOffEventLogger.cloneLoggerMonitor();
