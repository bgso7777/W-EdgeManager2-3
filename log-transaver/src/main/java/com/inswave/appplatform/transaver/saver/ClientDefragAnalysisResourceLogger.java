...
import com.inswave.appplatform.log.translate.ClientDefragAnalysisResource;
...
import com.inswave.appplatform.transaver.elasticsearch.dao.ClientDefragAnalysisResourceLogDailyRepository;
import com.inswave.appplatform.transaver.elasticsearch.dao.ClientDefragAnalysisResourceLogRepository;
...
@Service
public class ClientDefragAnalysisResourceLogger extends LoggerMonitor implements ElasticsearchService {
	...
	public static String className = "ClientDefragAnalysisResourceLogger";
	public static String currentIndexName = "ClientDefragAnalysisResourceLog";
	public static String dailyIndexName = "ClientDefragAnalysisResourceLogDaily";
...
//		Object objectNode = parse.getObjectNode(message);
		...
		for (Object object: jSONArray) {
			...
			super.setRowSize(super.getRowSize() + clientDefragAnalysisResourceLogOriginal.getClientDefragAnalysisResourceData().size());
			List<ClientDefragAnalysisResourceLog> clientDefragAnalysisResourceLogs = clientDefragAnalysisResource.reconstructDocument(clientDefragAnalysisResourceLogOriginal);
			List<ClientDefragAnalysisResourceLogDaily> clientDefragAnalysisResourceLogDailies = clientDefragAnalysisResource.reconstructDailyDocument(clientDefragAnalysisResourceLogOriginal);
			super.endTimeObjectConvert();

			// deviceId별 current 로그 저장
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			super.beginTimeElasticsearchCurrentInsert();
			ClientDefragAnalysisResourceLogRepository clientDefragAnalysisResourceLogRepository = (ClientDefragAnalysisResourceLogRepository) ElasticsearchDaoPackage.getInstance().getElasticsearchDao(currentIndexName);
			DynamicIndexBean.setIndexName(currentIndexName.toLowerCase());
			clientDefragAnalysisResourceLogRepository.setIndexName(currentIndexName.toLowerCase());
			if (!elasticsearchHighLevelClient.existIndices(currentIndexName.toLowerCase())) {
				do {
					try {
						clientDefragAnalysisResourceLogs.get(0).setIndexName(currentIndexName.toLowerCase());
						clientDefragAnalysisResourceLogRepository.setIndexName(currentIndexName.toLowerCase());
						clientDefragAnalysisResourceLogRepository.createIndex(clientDefragAnalysisResourceLogs.get(0));
			...
			try {
//				elasticsearchHighLevelClient.deleteDocument(currentIndexName.toLowerCase(), "deviceId", clientDefragAnalysisResourceLogOriginal.getDeviceId());
				if(!isCurrentIndexDelete) {
					elasticsearchHighLevelClient.deleteCurrentIndex(currentIndexName.toLowerCase(),clientDefragAnalysisResourceLogOriginal);
			...
			super.setCurrentRowSize(super.getCurrentRowSize() + clientDefragAnalysisResourceLogs.size());
			...
			for (ClientDefragAnalysisResourceLog clientDefragAnalysisResourceLog : clientDefragAnalysisResourceLogs) {
				clientDefragAnalysisResourceLog.setIndexName(currentIndexName.toLowerCase());
				document2s.add(clientDefragAnalysisResourceLog);
			}
			try {
			...
			// 일자별 로그 저장
			////////////////////////////////////////////////////////////////////////////////////////////////////////////
			if (Config.getInstance().getLog().getIndexDailylogClientDefraganAlysisResourceLogSavedDay()>0) {
				super.beginTimeElasticsearchDailyInsert();
				ClientDefragAnalysisResourceLogDailyRepository clientDefragAnalysisResourceLogDailyRepository = (ClientDefragAnalysisResourceLogDailyRepository) ElasticsearchDaoPackage.getInstance().getElasticsearchDao(dailyIndexName);
				String ip = "", deviceId = "", currentDate = "", timeCreated = "", regDate = "";
				document2s = new ArrayList<>();
				HashSet<String> indexNames = new HashSet<>();
				for (ClientDefragAnalysisResourceLogDaily clientDefragAnalysisResourceLogDaily : clientDefragAnalysisResourceLogDailies) {
					try {
						indexName = DateTimeConvertor.getIndexName(dailyIndexName.toLowerCase(), clientDefragAnalysisResourceLogDaily.getTimeCurrent());
						indexNames.add(indexName);
						clientDefragAnalysisResourceLogDaily.setIndexName(indexName);
						clientDefragAnalysisResourceLogDaily.setTimeRegistered(DateTimeConvertor.getTimeRegistered());
						ip = clientDefragAnalysisResourceLogDaily.getIp();
						deviceId = clientDefragAnalysisResourceLogDaily.getDeviceId();
						currentDate = DateTimeConvertor.getPrintDate(clientDefragAnalysisResourceLogDaily.getTimeCurrent());
						timeCreated = DateTimeConvertor.getPrintDate(clientDefragAnalysisResourceLogDaily.getTimeCreated());
						regDate = DateTimeConvertor.getPrintDate(clientDefragAnalysisResourceLogDaily.getTimeRegistered());
						document2s.add(clientDefragAnalysisResourceLogDaily);
					} catch (Exception e) {
						e.printStackTrace();
						super.setExceptionCount(super.getExceptionCount() + 1);
					}
				}
				super.setDailyRowSize(super.getDailyRowSize() + clientDefragAnalysisResourceLogDailies.size());
				try {
					BulkResponse bulkResponse = elasticsearchHighLevelClient.writeDocument2s(document2s);
					if (bulkResponse.hasFailures())
						logger.error(bulkResponse.buildFailureMessage());
	...
	public static void main(String[] argv) {
		...
		try	{
			messageFile = FileUtil.getFileToStringBuffer("C:\\Temp\\ClientDefragAnalysisResourceLog.message"); //
...
//				objectMapper.setDateFormat(dateFormat);
				ClientDefragAnalysisResourceLog clientDefragAnalysisResourceLog = objectMapper.readValue(objectNode.toString(), ClientDefragAnalysisResourceLog.class);
				System.out.println();
				for(int j = 0; j< ConstantsTranSaver.TAG_MAX_TERMINAL_COUNT; j++) {
					ClientDefragAnalysisResourceLog newClientDefragAnalysisResourceLog = objectMapper.readValue(objectNode.toString(), ClientDefragAnalysisResourceLog.class);
					newClientDefragAnalysisResourceLog.setDeviceId( newClientDefragAnalysisResourceLog.getDeviceId()+j );
				}
			}
		} catch(Exception e) {
			e.printStackTrace();
