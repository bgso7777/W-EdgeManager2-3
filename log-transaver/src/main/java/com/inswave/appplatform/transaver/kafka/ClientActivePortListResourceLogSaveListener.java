...
import com.inswave.appplatform.transaver.saver.ClientActivePortListResourceLogger;
...
@Service
public final class ClientActivePortListResourceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientActivePortListResourceLogger clientActivePortListResourceLogger;

    private static final String topicName = "ClientActivePortListResourceLog";
    private static final String groupId = "ClientActivePortListResourceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientactiveportlistresourcelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientActivePortListResourceLogger.insert("ClientActivePortListResourceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientActivePortListResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientActivePortListResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientActivePortListResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientActivePortListResourceLogger.cloneLoggerMonitor();
