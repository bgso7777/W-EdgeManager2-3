...
import com.inswave.appplatform.transaver.saver.ClientMBRResourceLogger;
...
@Service
public final class ClientMBRResourceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientMBRResourceLogger clientMBRResourceLogger;

    private static final String topicName = "ClientMBRResourceLog";
    private static final String groupId = "ClientMBRResourceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientmbrresourcelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientMBRResourceLogger.insert("ClientMBRResourceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientMBRResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientMBRResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientMBRResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientMBRResourceLogger.cloneLoggerMonitor();
