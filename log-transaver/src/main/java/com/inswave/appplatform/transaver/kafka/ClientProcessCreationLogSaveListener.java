...
import com.inswave.appplatform.transaver.saver.ClientProcessCreationLogger;
...
@Service
public final class ClientProcessCreationLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientProcessCreationLogger clientProcessCreationLogger;

    public static final String topicName = "ClientProcessCreationLog";
    public static final String groupId = "ClientProcessCreationLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientprocesscreationlogsavelistener.enabled}",containerFactory="kafkaClientProcessCreationLogSaveListenerContainerFactory")
   public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
       ...
       try {
           ...
           clientProcessCreationLogger.insert("ClientProcessCreationLog", consumerRecord.value());

           super.setProcessByte((long) consumerRecord.value().getBytes().length);

       } catch (JsonProcessingException e) {
           e.printStackTrace();
           clientProcessCreationLogger.setJsonProcessingException(true);
       } catch (Exception e) {
           e.printStackTrace();
           clientProcessCreationLogger.setException(true);
       } finally {
           try {
               consumer.commitSync();
           } catch (Exception e) {
               e.printStackTrace();
               clientProcessCreationLogger.setCommitSyncException(true);
           } finally {
           }
       }
       super.end();
       JavaMelodyMonitor.printInfo( this.getClass().getSimpleName(),"end runningTime-->"+super.getRunningTime()+ " processCount->"+super.getProcessCount()+" bps-->"+super.getBps()+" bpms-->"+super.getBpms()+" processByte->"+super.getProcessByte());
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientProcessCreationLogger.cloneLoggerMonitor();
