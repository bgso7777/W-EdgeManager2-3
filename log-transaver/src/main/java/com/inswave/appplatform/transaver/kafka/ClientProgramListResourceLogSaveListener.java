...
import com.inswave.appplatform.transaver.saver.ClientProgramListResourceLogger;
...
@Service
public final class ClientProgramListResourceLogSaveListener extends KafkaListenerMonitor {

    @Autowired
    ClientProgramListResourceLogger clientProgramListResourceLogger;

    private static final String topicName = "ClientProgramListResourceLog";
    private static final String groupId = "ClientProgramListResourceLogSaverGroup1";

    @KafkaListener(topics=topicName,groupId=groupId,autoStartup="${kafka.listener.clientprogramlistresourcelogsavelistener.enabled}",containerFactory="kafkaListenerContainerFactory")
    public void consume(ConsumerRecord<String, String> consumerRecord, Consumer<String, String> consumer) {
        ...
        try {
            ...
            clientProgramListResourceLogger.insert("ClientProgramListResourceLog", consumerRecord.value());

            super.setProcessByte((long) consumerRecord.value().getBytes().length);

        } catch (JsonProcessingException e) {
            e.printStackTrace();
            clientProgramListResourceLogger.setJsonProcessingException(true);
        } catch (Exception e) {
            e.printStackTrace();
            clientProgramListResourceLogger.setException(true);
        } finally {
            try {
                consumer.commitSync();
            } catch (Exception e) {
                e.printStackTrace();
                clientProgramListResourceLogger.setCommitSyncException(true);
            } finally {
            }
        }
        super.end();
        ...
        if(Config.getInstance().getLog().getIsUseModuleMonitoring()) {
            try {
                LoggerMonitor loggerMonitor = clientProgramListResourceLogger.cloneLoggerMonitor();
